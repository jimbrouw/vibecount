with glossary_seed(term, explanation, example, sort_order) as (
  values
    (
      'Self Assessment',
...
updated as (
  ...
  from glossary_seed gs
  where lower(btrim(gt.term)) = lower(btrim(gs.term))
  returning lower(btrim(gt.term)) as normalized_term
)
insert into public.glossary_terms (term, explanation, example, sort_order)
select gs.term, gs.explanation, gs.example, gs.sort_order
from glossary_seed gs
where not exists (
  select 1
  from updated u
  where u.normalized_term = lower(btrim(gs.term))
)
and not exists (
  select 1
