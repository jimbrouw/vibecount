create table public.csv_import_rows (
  ...
  constraint csv_import_rows_type_valid check (record_type in ('income', 'expense')),
  constraint csv_import_rows_status_valid check (
    status in ('review', 'approved', 'discarded', 'committed', 'invalid')
  )
);

alter table public.csv_import_rows
  add constraint csv_import_rows_import_same_user_fk
  foreign key (import_id, user_id)
  references public.record_imports (id, user_id)
  on delete cascade;

alter table public.csv_import_rows
  add constraint csv_import_rows_category_fk
  foreign key (category_id)
  references public.record_categories (id)
  on delete set null;

alter table public.csv_import_rows
  add constraint csv_import_rows_committed_record_fk
...
create index csv_import_rows_user_import_idx
  on public.csv_import_rows (user_id, import_id, row_number);

create index csv_import_rows_user_status_idx
  on public.csv_import_rows (user_id, status, created_at desc);
...
create policy "Users can read their own csv import rows"
...
create policy "Users can update their own csv import rows"
on public.csv_import_rows
for update
to authenticated
using (auth.uid() = user_id)
...
create policy "Users can delete their own csv import rows"
on public.csv_import_rows
for delete
to authenticated
using (auth.uid() = user_id);
